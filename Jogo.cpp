...
void jogarNivel(vector<vector<char>> labirinto, int &totalPassos) {
    ...
    gameRunning = true; // Ativa o loop de obstáculos
    std::thread obstaculosThread(movimentarObstaculos, std::ref(labirinto), std::ref(semaforo));

    printLabirinto(labirinto, contador, totalItens, passos);

    while (true) {
        char comando = getch(); // Pega o comando do jogador
        semaforo.lock();
        if (processarComando(labirinto, comando, jogadorX, jogadorY, contador, passos, totalPassos, totalItens)) {
            semaforo.unlock();
            break; // Nível concluído
        }
        semaforo.unlock();
        printLabirinto(labirinto, contador, totalItens, passos);
    }

    gameRunning = false; // Encerra a thread
    obstaculosThread.join(); // Aguarda thread terminar
